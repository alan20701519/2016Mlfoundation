...
*/
void SGD(double *w, Data data){
	double gradient[DIMENSION+1] = {0.0};
	double temp = sigmoid(-1 * vectorMul(w,data.x) * (double)data.y);
	for(int i = 0; i < DIMENSION+1; i++)
		gradient[i] += -1.0 * temp * data.x[i] * data.y; 
	for(int i = 0; i < DIMENSION+1; i++)
		w[i] = w[i] - LEARN_RATE * gradient[i];
	
}
void logisticRegression(double *w, vector<Data> data){
	for(int i = 0; i < ITERATION; i++){
		SGD(w, data[i%data.size()]);
	}
}
